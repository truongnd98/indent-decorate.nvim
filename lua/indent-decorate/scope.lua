...
local DEFAULT_OPTS = {
  ...
  filter = function(buf)
    return vim.bo[buf].buftype == "" and vim.b[buf].indent_decorate_scope ~= false and vim.g.indent_decorate_scope ~= false
  end,
  ...
  -- using the `scope.textobject` and `scope.jump` functions.
  keys = {
    ---@type table<string, scope.TextObject|{desc?:string}>
    textobject = {
      ii = {
    ...
    ---@type table<string, scope.Jump|{desc?:string}>
    jump = {
      ["[i"] = {
...
---@alias scope.scope {buf: number, from: number, to: number, indent?: number}
...
---@field to number
---@field indent? number
---@field opts scope.Opts
local Scope = {}
Scope.__index = Scope

---@generic T: scope.Scope
---@param self T
---@param scope scope.scope
---@param opts scope.Opts
---@return T
...
function Scope:with(opts)
  ...
  return setmetatable(opts, getmetatable(self)) --[[ @as scope.Scope ]]
end
...
function Scope:expand(line)
  local ret = self ---@type scope.Scope?
  while ret do
...
---@class scope.IndentScope: scope.Scope
...
---@class scope.TSScope: scope.Scope
---@field node TSNode
...
function TSScope:with_edge()
  local ret = self ---@type scope.TSScope?
  while ret do
...
function TSScope:with(opts)
  local ret = Scope.with(self, opts) --[[ @as scope.TSScope ]]
  return ret:fix()
end
...
function TSScope:init(cb, opts)
  ...
  id_utils.parse(parser, opts.treesitter.injections, cb)
end
...
---@param cb fun(scope?: scope.Scope)
---@param opts? scope.Opts|{parse?:boolean}
function M.get(cb, opts)
  opts = M.get_config(opts) --[[ @as scope.Opts ]]
  ...
  ---@type scope.Scope
  local Class = (opts.treesitter.enabled and id_utils.get_lang(opts.buf)) and TSScope or IndentScope
  ...
  local scope = Class:find(opts) --[[ @as scope.Scope? ]]
  ...
  if scope and opts.end_pos and not vim.deep_equal(opts.pos, opts.end_pos) then
    local end_scope = Class:find(vim.tbl_extend("keep", { pos = opts.end_pos }, opts)) --[[ @as scope.Scope? ]]
  ...
  if scope then
    local s = scope --- @type scope.Scope?
    while s do
    ...
    -- expand with edge
    if opts.edge then
      scope = scope:with_edge() --[[@as scope.Scope]]
    end
  end
...
---@class scope.Listener
---@field id integer
---@field cb scope.Attach.cb
---@field opts scope.Config
---@field dirty table<number, boolean>
---@field timer uv.uv_timer_t
---@field augroup integer
---@field enabled boolean
---@field active table<number, scope.Scope>
local Listener = {}

---@param cb scope.Attach.cb
---@param opts? scope.Config
function Listener.new(cb, opts)
  ...
  self.enabled = false
  self.opts = M.get_config(opts) --[[ @as scope.Opts ]]
  id = id + 1
  self.id = id
  self.active = {}
  return self
...
function Listener:enable()
  ...
  self.enabled = true
  self.augroup = vim.api.nvim_create_augroup("indent_decorate_scope_" .. self.id, { clear = true })
...
---@param opts? scope.TextObject
function M.textobject(opts)
  opts = M.get_config(opts) --[[ @as scope.TextObject ]]
...
---@param opts? scope.Jump
function M.jump(opts)
  opts = M.get_config(opts) --[[ @as scope.Jump ]]
  M.get(function(scope)
    if not scope then
      return
    end
